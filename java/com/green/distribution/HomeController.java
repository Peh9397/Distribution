...
public class HomeController {
	...
	public void login(Employee employee, Model model, HttpSession session) {
		...
		if (employee2 == null || employee2.getDel().equals("y"))
			result = -1; // 없는cd
		else if (employee2.getPASSWORD().equals(employee.getPASSWORD())) {
			result = 1; // 성공(일치)
			session.setAttribute("EMPCD", employee2.getEMPCD());
			session.setAttribute("ENAME", employee2.getENAME());
			session.setAttribute("JOB", employee2.getJOB());
		}
